...
- `data-migrate`：运行 Session、Knowledge、Artifact 在线迁移，管理捕获、校验、切换与回滚。
...
平台提供三项相互配合的能力：租户独立组合 Session/Memory 后端并选择 Knowledge/Artifact 实例；通过持久化捕获、完整记录校验和回滚窗口迁移状态；通过消息身份、固定执行版本与 lease/fence 使 Worker 跨节点恢复。业务可在同一套基础设施中部署不同 Agent，并沿 trace、执行记录和审计确定故障影响范围与恢复步骤。
