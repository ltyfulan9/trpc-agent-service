...
V14 已发布到 [GitHub](https://github.com/ltyfulan9/trpc-agent-service/tree/v14-final)，固定发布标签为 `v14.0.1-final`；仓库默认分支是 `main`，V14 发布分支是 `v14-final`。评委可在全新目录执行 `git clone`、`git checkout v14.0.1-final`、`git rev-parse v14.0.1-final^{}`、`./scripts/validate.sh`、Compose config 和镜像构建。仓库保留 Apache-2.0 LICENSE、`.github/workflows/verify.yml` 和不含秘密的测试配置；真实企微回调仍按本页 `EXTERNAL_REQUIRED` 边界验收。
