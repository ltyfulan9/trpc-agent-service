...
Knowledge 以租户和应用隔离向量与检索，Artifact 把不可变版本元数据放在 PostgreSQL、正文放在对象存储。受控知识导入由 `pkg/knowledgeingest` 提供平台层原语：输入限制为 UTF-8 文本、1 MiB 和 256 个分片，使用稳定 source/chunk 身份、内容指纹和幂等更新；存储打开器必须返回已绑定 tenant/app 且经过迁移装饰器的 VectorStore，不接受请求携带 DSN 或凭据。元数据和对象没有跨存储原子提交，通过稳定对象身份、哈希校验、删除标记和恢复流程收敛。检索排名质量、向量维度与模型兼容性另行验收；记录内容相等只能证明数据同步，不能证明业务查询效果相同。
...
容量按入口峰值、模型时长、并发余量与下游配额确定。`CONTROL_DB_MAX_CONNECTIONS` 在同一数据库上分摊为独立的元数据、执行围栏和迁移门禁池，避免持锁请求耗尽其后续查询所需连接；租户后端连接另计。连接池由运行入口持有，在运行时与存储排空后关闭。扩容同时核算 Redis、向量检索及提供方配额；压测覆盖模型变慢、通道限流、数据库抖动、重试放大和热点会话，检查吞吐、尾延迟、队列年龄、连接等待与恢复能力，具体预算见[部署契约](../deploy/kubernetes/README.md#数据库连接预算与关闭顺序)。
