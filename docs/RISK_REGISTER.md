...
评分格式为影响/概率（1–5）。表中记录每项风险的观测信号、已实现控制、恢复路径和负责模块；环境验收按 [EXTERNAL_ACCEPTANCE_RUNBOOK.md](EXTERNAL_ACCEPTANCE_RUNBOOK.md) 执行。
...
| R-26 | HA 切换、PITR 或备份无法恢复 | 5/2 | RPO/RTO 超标 | 定期 restore drill、WAL/Redis 策略、对象版本、运行手册与责任人 | 外部必验 / SRE |
| R-27 | 成本/吞吐估算低于真实长上下文峰值 | 4/3 | queue age、token/min、P95/P99 | 用真实 payload 阶梯压测；按限流/连接池/HPA 指标扩容；租户 quota | 外部必验 / SRE |
| R-28 | 过长 PostgreSQL Session schema/prefix 使索引名超过 63 字节并被截断 | 4/2 | Session Service 启动 schema verification 失败 | operator profile 限制短固定 schema/prefix；保留上游 fail-closed 校验；发布前真实初始化 | Profile 配置约束 / Storage |
| R-29 | 租户借 MCP URL/Header 越权出网或一个 MCP 故障拖垮全体租户 | 5/3 | 非预期 egress、profile init error、Tool P95 | URL/Header/profile 仅 operator-owned；禁 stdio/危险 Header；精确 Tool allowlist；按使用 profile 延迟初始化；目标 NetworkPolicy/DNS/SLA 验收 | 源码闭环；目标 MCP 待外部 / Platform |
